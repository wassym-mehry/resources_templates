...
{{/unless}}
{{#unless cidr_block_variable}}
{{!-- Don't create cidr_block variable when using computed value --}}
{{else}}
...
{{/unless}}
{{#unless cidr_block_variable}}
cidr_block        = {{{cidr_block}}}  {{!-- Use computed CIDR directly --}}
{{else}}
cidr_block        = var.{{name}}_cidr_block
{{/unless}}
{{#unless availability_zone_variable}}
availability_zone = {{{availability_zone}}}  {{!-- Use computed or static AZ directly --}}
{{else}}
...
{{#if vpc_id_variable}}
{{else}}
depends_on = [module.{{#if vpc_module_name}}{{vpc_module_name}}{{else}}vpc1{{/if}}]
{{/if}}
}
/* MAIN_TF_END */
/* OUTPUTS_TF_START */
